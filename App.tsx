...
const App: React.FC = () => {
  ...
  const [products, setProducts] = useState<HoneyProduct[]>(() => {
    ...
    try {
      const saved = localStorage.getItem('premium_asallar_v1');
  ...
  useEffect(() => {
    localStorage.setItem('premium_asallar_v1', JSON.stringify(products));
  }, [products]);
        ...
        <div className="container mx-auto px-6 md:px-12 relative z-10 lg:w-3/5">
          <div className="reveal">
            <div className="flex items-center gap-4 mb-8">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.5em] text-[#C5A059]">ESTABLISHED 2022</span>
            </div>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold text-[#1A1A1A] mb-8 leading-[1.1] md:leading-[0.9]">
              {UI_TEXT.hero_title_1[locale]} <br/> 
              <span className="italic font-normal text-[#C5A059]">{UI_TEXT.hero_title_2[locale]}</span>
            </h1>
            <p className="text-lg md:text-xl text-stone-500 mb-10 max-w-lg leading-relaxed font-light">
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <a href="#collection" className="group relative px-10 py-5 overflow-hidden border border-[#1A1A1A] text-center min-w-[200px]">
                ...
                <span className="relative z-10 text-[10px] font-bold uppercase tracking-[0.3em] text-[#1A1A1A] group-hover:text-white">
                  {UI_TEXT.explore_btn[locale]}
                </span>
              </a>
              <a href={`tel:${CONTACT.phone}`} className="flex items-center gap-4 group">
                <div className="w-12 h-12 rounded-full border border-stone-200 flex items-center justify-center group-hover:border-[#C5A059] transition-all bg-white">
                  <i className="fa-solid fa-phone text-stone-400 group-hover:text-[#C5A059]"></i>
                </div>
                <div>
                   <div className="text-[9px] font-bold uppercase tracking-widest text-stone-400">Order Hotline</div>
      ...
      <main id="collection" className="py-24 md:py-32 px-6 md:px-12 bg-white">
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 md:gap-12">
            {products.map((honey) => (
              <HoneyCard key={honey.id} honey={honey} locale={locale} onOrder={handleOrder} />
            ))}
          </div>
        </div>
      </main>

      <section className="bg-[#1A1A1A] py-32 px-6 text-center text-white relative overflow-hidden">
        <div className="relative z-10 max-w-3xl mx-auto">
          <div className="w-16 h-px bg-[#C5A059] mx-auto mb-12"></div>
          <h2 className="text-4xl md:text-6xl font-bold mb-10 leading-tight">{UI_TEXT.cta_title[locale]}</h2>
          <p className="text-base md:text-lg text-stone-400 mb-12 font-light max-w-xl mx-auto">{UI_TEXT.cta_desc[locale]}</p>
          <div className="flex flex-col sm:flex-row justify-center gap-6">
            <button 
              onClick={() => window.open(`https://t.me/${CONTACT.telegram}`, '_blank')}
              className="bg-[#C5A059] text-white px-10 py-5 font-bold tracking-[0.3em] text-[10px] uppercase hover:bg-white hover:text-[#1A1A1A] transition-all"
            >
              Order via Telegram
            </button>
            <a href={`https://www.instagram.com/${CONTACT.instagram}/`} target="_blank" rel="noopener noreferrer" className="border border-stone-700 text-stone-400 px-10 py-5 font-bold tracking-[0.3em] text-[10px] uppercase hover:border-[#C5A059] hover:text-[#C5A059] transition-all">
              Instagram
            </a>
          </div>
        </div>
